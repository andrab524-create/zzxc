...
client.once('ready', async () => {
    ...
    console.log("Slash Vorakey registered!");
});
...
client.on('interactionCreate', async (interaction) => {
    try {
        ...
        // Slash Command: /syncvip
        if (interaction.isChatInputCommand() && interaction.commandName === 'syncvip') {
            if (!interaction.member.roles.cache.has(STAFF_ROLE_ID)) {
                return interaction.reply({ content: "Hanya staff dengan role khusus!", ephemeral: true });
            }
            ...
            if (!interaction.guild) {
                return interaction.editReply({ content: "Command ini hanya bisa dipakai di server!" });
            }

            // Fetch semua member dengan Premium role
            await interaction.guild.members.fetch();
            const premiumMembers = interaction.guild.members.cache.filter(m => m.roles.cache.has(PREMIUM_ROLE_ID));
            ...
            for (const [memberId, member] of premiumMembers) {
                // Cek apakah sudah ada di whitelist atau punya key
                const [whitelistDoc, keysSnap] = await Promise.all([
                    db.collection('whitelist').doc(memberId).get(),
                    db.collection('keys').where('userId', '==', memberId).limit(1).get()
                ]);

                if (whitelistDoc.exists || !keysSnap.empty) {
                    skipped++;
                    continue;
                }
                ...
                batch.set(db.collection('whitelist').doc(memberId), {
                    userId: memberId,
                ...
                await batch.commit();
                userKeyCache.delete(memberId);

                results.push(`✅ ${member.user.tag}`);
                added++;

                await logAction("AUTO WHITELIST", interaction.user.tag, member.user.tag, "Sync VIP", `Key: ${newKey}`);
            }
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: "Skipped (already has key)", value: `${skipped}`, inline: true }
                )
                .setColor(added > 0 ? "#00ff00" : "#ffff00")
                .setTimestamp();

            if (results.length > 0 && results.length <= 20) {
                embed.addFields({ name: "New Whitelisted", value: results.join("\n"), inline: false });
            }

            return interaction.editReply({ embeds: [embed] });
        }

        // Slash Command: /listvip
        if (interaction.isChatInputCommand() && interaction.commandName === 'listvip') {
            if (!interaction.member.roles.cache.has(STAFF_ROLE_ID)) {
                return interaction.reply({ content: "Hanya staff dengan role khusus!", ephemeral: true });
            }
            ...
            for (const [memberId, member] of premiumMembers) {
                ...
                if (whitelistDoc.exists || !keysSnap.empty) {
                    hasKey.push(member.user.tag);
                } else {
                    noKey.push(`❌ ${member.user.tag}`);
                }
            }
            ...
            if (noKey.length > 0) {
                ...
                embed.addFields({ name: "✅ Status", value: "Semua VIP sudah punya key!", inline: false });
            }
