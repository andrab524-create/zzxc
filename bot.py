...
def add_ticket(user_id, channel_id, is_premium=False):
    """Add a ticket with premium status"""
...
def get_ticket_channel(user_id):
    """Get channel ID from ticket data (supports old and new format)"""
    data = active_tickets.get(user_id)
    if data is None:
        return None
    if isinstance(data, dict):
        return data.get("channel_id")
    return data  # Legacy format: just channel_id

def is_ticket_premium(channel_id):
    """Check if a ticket channel is premium"""
    for user_id, data in active_tickets.items():
        if isinstance(data, dict):
            if data.get("channel_id") == channel_id:
                return data.get("is_premium", False)
        elif data == channel_id:
            # Legacy format - default to False, but check channel category
            return False
    return False
...
class DoneButtonView(ui.View):
    ...
    async def done_button(self, interaction: Interaction, button: ui.Button):
        ...
        # Find ticket creator (support both old and new format)
        ticket_creator_id = None
        for uid, data in active_tickets.items():
            # Support new format (dict with channel_id) and old format (just channel_id)
            cid = data.get("channel_id") if isinstance(data, dict) else data
            if cid == channel.id:
        ...
        # Determine sale amount based on ticket type - CHECK FROM STORED DATA
        # Use is_ticket_premium function which reads from stored data
        ticket_is_premium = is_ticket_premium(channel.id)
        
        # Fallback: if legacy format or not found, check channel category
        if not ticket_is_premium and channel.category_id in [TICKET_CATEGORY_ID, TICKET2_CATEGORY_ID]:
            # Check channel name for "premium" hint or just assume premium for main ticket categories
            ticket_is_premium = True
        
        sale_amount = 30000 if ticket_is_premium else 0
...
class TicketControlView(ui.View):
    ...
    async def claim_ticket_callback(self, interaction: Interaction):
        ...
        for uid, data in ticket_source.items():
            # Support new format (dict with channel_id) and old format (just channel_id)
            cid = data.get("channel_id") if isinstance(data, dict) else data
            if cid == channel.id:
                ticket_creator_id = uid
    ...
    async def close_ticket_callback(self, interaction: Interaction):
        ...
        if self.is_x8:
             ...
             for uid, data in list(active_tickets.items()):
                # Support new format (dict with channel_id) and old format (just channel_id)
                cid = data.get("channel_id") if isinstance(data, dict) else data
                if cid == channel.id:
...
async def create_ticket(interaction: Interaction, category_name: str):
    ...
    if is_x8:
        ...
        if user.id in active_tickets:
            # Use get_ticket_channel to support both old and new format
            channel_id = get_ticket_channel(user.id)
            ...
            if ch:
                ...
                remove_claim(channel_id)
                remove_done_ticket(channel_id)
    ...
    # Determine if this is a premium ticket BEFORE saving
    ...
    if is_x8:
        ...
        add_ticket(user.id, ticket_channel.id, is_premium=is_premium)
...
class Client(commands.Bot):
    ...
    async def on_ready(self):
        ...
        try:
            ...
            print(f"✅ Globally vorahubBOTs synced {len(synced)} slash commands.")
        except Exception as e:
        ...
        # Update semua panel di ticket aktif (Claim/Close/Done buttons)
        await self.update_all_ticket_panels()
    ...
    async def update_all_ticket_panels(self):
        """Update semua panel di ticket aktif saat bot restart"""
        if not self.guilds:
            return
        ...
        for user_id, data in list(active_tickets.items()):
            try:
                ...
                # Fallback: check category for premium status
                if not is_premium and channel.category_id in [TICKET_CATEGORY_ID, TICKET2_CATEGORY_ID]:
                    is_premium = True
                
                # Find and update control panel message (with Claim/Close buttons)
                async for msg in channel.history(limit=30):
                    if msg.author.id == self.user.id and msg.embeds:
                        for emb in msg.embeds:
                            # Update ticket control panel
                            if "Ticket Dibuat" in (emb.title or ""):
                                await msg.edit(view=TicketControlView(is_premium=is_premium, is_x8=False))
                                print(f"[TICKET-PANEL] ✓ Updated control panel di {channel.name}")
                                updated_count += 1
                            # Update Done panel
                            elif "Whitelist Berhasil" in (emb.title or ""):
                                await msg.edit(view=DoneButtonView(is_premium=is_premium))
                                print(f"[TICKET-PANEL] ✓ Updated Done panel di {channel.name}")
                                updated_count += 1
                            # Update Premium Purchase / Payment panel
                            elif "Premium Purchase" in (emb.title or ""):
                                await msg.edit(view=PaymentActionView())
                                print(f"[TICKET-PANEL] ✓ Updated Payment panel di {channel.name}")
                                updated_count += 1
            except Exception as e:
                print(f"[TICKET-PANEL] ✗ Error updating ticket {channel_id}: {e}")
        
        # Update panel di ticket X8
        for user_id, channel_id in list(x8_tickets.items()):
            try:
                channel = guild.get_channel(channel_id)
                if not channel:
                    continue
                ...
                async for msg in channel.history(limit=30):
                    if msg.author.id == self.user.id and msg.embeds:
                        for emb in msg.embeds:
                            if "Ticket Dibuat" in (emb.title or ""):
                                await msg.edit(view=TicketControlView(is_premium=False, is_x8=True))
                                print(f"[TICKET-PANEL] ✓ Updated X8 control panel di {channel.name}")
                                updated_count += 1
                                break
            except Exception as e:
                print(f"[TICKET-PANEL] ✗ Error updating X8 ticket {channel_id}: {e}")
        
        # Update panel di ticket Midman
        for user_id, channel_id in list(midman_tickets.items()):
            try:
                channel = guild.get_channel(channel_id)
                if not channel:
                    continue
                ...
                async for msg in channel.history(limit=30):
                    if msg.author.id == self.user.id and msg.embeds:
                        for emb in msg.embeds:
                            if "Midman Ticket Dibuat" in (emb.title or ""):
                                await msg.edit(view=MidmanTicketControlView())
                                print(f"[TICKET-PANEL] ✓ Updated Midman control panel di {channel.name}")
                                updated_count += 1
                                break
            except Exception as e:
                print(f"[TICKET-PANEL] ✗ Error updating Midman ticket {channel_id}: {e}")
        
        print(f"[TICKET-PANEL] ✓ Total {updated_count} panel berhasil di-update")
    ...
    async def on_message(self, message: dc.Message):
        ...
        if "You have been whitelisted! You can access the script via this message" in message.content:
            ...
            # Get all channel_ids from active_tickets (support new format)
            ticket_channel_ids = [
                d.get("channel_id") if isinstance(d, dict) else d 
                for d in active_tickets.values()
            ]
