...
local function updateNotifColor(notifLabel, value)
...
local YttaTab = Window:CreateTab({
	Name = "YTTA",
	Icon = "rbxassetid://7733765398" -- Same icon as exclusive or generic
})
...
-- ================================================================
-- 🧪 BLATANT EXPERIMENTAL (YTTA)
-- ================================================================
YttaTab:CreateSection({ Name = "Blatant Experimental" })

local BlatantExperimental = {}
BlatantExperimental.Active = false
BlatantExperimental.WaitCast = 0.001

-- Main spam loop for Experimental
function Exp_spamLoop()
    while BlatantExperimental.Active do
        local currentTime = tick()
        ...
        -- Wait Cast (User Input) - Delay before completing
        task.wait(BlatantExperimental.WaitCast)
        
        UB_safeFire(function()
            UB_RE_FishingCompleted:FireServer()
        end)
        
        -- Instant Cancel (No extra delay)
        UB_safeFire(function()
            UB_RF_CancelFishingInputs:InvokeServer()
        end)
    end
...
BlatantExperimental.Start = function()
    if BlatantExperimental.Active then return end
    BlatantExperimental.Active = true
    task.spawn(Exp_spamLoop)
end

BlatantExperimental.Stop = function()
    if not BlatantExperimental.Active then return end
    BlatantExperimental.Active = false
...
YttaTab:CreateToggle({
    Name = "Blatant Experimental",
    Default = false,
    Callback = function(state)
        if state then
            BlatantExperimental.Start()
        else
            BlatantExperimental.Stop()
        end
    end
})
...
YttaTab:CreateInput({
    ...
    Callback = function(v)
        local n = tonumber(v)
        if n then
            BlatantExperimental.WaitCast = n
        end
    end
})
